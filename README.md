...
| **设置页「费用统计」** | 设置 → **费用统计**(在「侧边卡片」之后):**默认打开就是今天**的回复;点**日期**日历或**月份**日历可换范围(互斥,另有「清除筛选」与「刷新」)。逐条列出每一次回复的**时间/会话/用量/费用**(四列统一左对齐,¥ 与 $ 同行显示),**每页 15 条**并带上一页/下一页;顶部**费用合计跟随查询**(¥ 与 $ 同行,右侧回复/会话/用量居中显示)。数据由插件自己的宿主路由折遍所有会话日志得到 |
...
| 时刻 | 账户余额 | 独立复算累计 | Δ 余额 vs Δ 复算 |
|---|---|---|---|
| 11:07 | $9.03 | $3.50 | — |
| 12:20 | $8.78 | $3.75 | $0.25 vs **$0.249** ✓ |
| 15:03 | $8.00 | $4.33 | $0.78 vs **$0.575** ✗ 差 $0.205 |

第三行那 $0.205 的差额**已定位到具体原因**:本会话在 **14:17:15 触发了一次上下文压缩**
(`compaction/summary`,707,231 未缓存输入 + 4,896 输出,按当时高峰价 = **¥1.45 / $0.218**)。
压缩是一次独立的模型调用,既不属于任何一次回复,也不在官方「用量」胶囊里 ——
**两边的逐回合折算都看不见它**。把它加回来:`$0.575 + $0.218 = $0.793`,与余额下降 `$0.78`
相差 **$0.013**(在余额两位小数的舍入误差 ±$0.01 与标题生成调用之内)。

> 结论:**逐回合口径与账单一致到 1 美分左右;已知唯一的系统性缺口是「压缩」**——
> 长会话里每次压缩约 $0.2(高峰),多次压缩会让插件合计明显低于余额。
> 官方胶囊同样不显示这部分。
...
3. 口径提示:余额还会包含**标题生成、压缩**等少量模型调用,以及仍在进行的请求 —— 所以
   `余额下降 ≥ 插件合计` 是正常的。压缩是其中最贵的一项(实测一次约 $0.2),见上表。
