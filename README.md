...
| **设置页「费用统计」** | 设置 → **费用统计**(在「侧边卡片」之后):**默认打开就是今天**的回复;点**日期**日历或**月份**日历可换范围(互斥,另有「清除筛选」与「刷新」)。逐条列出每一次回复的**时间/会话/用量/费用**(四列统一左对齐,¥ 与 $ 同行显示),**每页 20 条**并带上一页/下一页;顶部**费用合计跟随查询**。数据由插件自己的宿主路由折遍所有会话日志得到 |
