...
| **设置页「费用统计」** | 设置 → **费用统计**(在「侧边卡片」之后):`按日 / 按月 / 全部回复` 查询,**逐条列出每一次回复**的时间/会话/用量/费用,点日/月行即筛选;顶部**费用合计跟随查询**。数据由插件自己的宿主路由折遍所有会话日志得到 |
...
- **顶部查询**:`按日` / `按月` / `全部回复` 三个切换,外加「取消选择」与「刷新」。
- **费用合计卡片**:始终跟随当前查询 —— 选中某一天/某一月时就是那一天/那一月的合计;显示 ¥ 主、$ 次,
  以及 回复数 / 会话数 / 用量 / 子代理数 / 未计价数。
- **分桶表**(按日/按月模式):日期或月份 / 回复数 / 用量 / 费用,点任意一行把下方明细筛到该桶,再点一次取消。
- **逐次回复明细表**:每一条助手回复一行 —— 时间(`MM-DD HH:mm`)、会话(带 `子代理` / `无价目` 标记)、用量、费用(¥ 主 / $ 次);
  悬停会话名可看到 `会话 · #回合 · 计费模型`。
...
pnpm test            # 31 项单测:计费/时段/别名规则 + 会话日志折叠 + 逐条统计模型(vitest)
...
src/
  ...
  client/
    ...
    StatsSection.tsx      设置页「费用统计」:查询切换、合计卡片、日/月分桶表、逐次回复明细表
    stats-model.ts        统计模型(纯函数):日/月键 → 分桶 → 筛选 → 合计
