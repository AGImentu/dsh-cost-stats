...
分页 20 条由纯函数 `paginate` 决定,合计卡片始终覆盖**整个选择**(不是当前页)。
...
| 统计页**默认筛选 = 今天**,并且分页而是滚动全部 | 打开页面第一眼想看的是"今天花了多少",而不是全部历史;固定的每页 20 条让列宽与行高稳定,长历史也不会变成一堵墙。分页算术是纯函数 `paginate`(越界夹取在渲染期完成),所以换筛选不会出现空白页或少一帧 |
